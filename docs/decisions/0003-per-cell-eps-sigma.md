...
- Toolbar grows two contextual sliders (εr appears when Dielectric is the active brush; σ appears when Lossy is). The four-button palette stays — it now sets *brush parameter defaults* rather than per-cell shader switches.
...
Slider ranges: εr ∈ [1, 12], σ ∈ [0, 2]. εr cap is a UX call — the math is stable beyond 12, but visualization tint saturates and the wavelength compresses past the point of being useful in a 1024² grid.
...
- **Three-mode UI (Eraser / PEC / Material) + two always-visible sliders**: more expressive but louder visually. Keeping the four-button palette with one *contextual* slider preserves the M3 look and adds the slider only when it's meaningful.
